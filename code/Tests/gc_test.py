...
def pause(file,message):
        write_line(file,f"M117 {message}", message)
        write_line(file, "M226", "paused")
...
def asp_dsp_cycle(file,trial): # pipette cycle to enter perform and action and exit well
    ...
    pause(file,f"trial {trial+1} pre-measure: please tare scale and continue")
    write_line(file, f"G1 E{E_FIRST_STOP}",  "dispense")
    write_line(file, f"G1 E{E_SECOND_STOP} F300",  "dispense")
    pause(file,f"trial {trial+1} measure: please record mass on sclae and continue")
